...
public class Java2DMenus extends Frame
{
	...
	private Base selected = null;
	private ArrayList<Base> primitives = new ArrayList<Base>();
		...
		addMouseListener(
				new MouseAdapter()
				{
					...
					// select mode 
						...
						for(Base i : primitives) {
								// ---------- bounding box test -----------
							 if(i.boundingBox(evt.getX(), evt.getY()) == true) {
								selected = i;
								selected.set_select(true);	// flags that a primitive was selected 
								break;
							}
							 else {
								selected = null; 
